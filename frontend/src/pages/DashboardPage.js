...
import { Activity, AlertTriangle, Calendar, Clock, User, Video, TrendingUp, History, ShieldAlert, MapPin, Image as ImageIcon, X, ArrowRight } from "lucide-react"
...
function DashboardPage() {
  ...
  const generateChartData = () => {
    ...
    // Take up to 8 most recent events, reverse to chronological (oldest to newest left to right)
  ...
  useEffect(() => {
    ...
    async function checkActiveRide() {
      try {
        if (isHistoricalView) {
            const res = await fetch(`http://localhost:5001/get_ride_details?ride_id=${activeRideId}`)
            if (res.ok) {
            ...
            const res = await fetch(`http://localhost:5001/get_active_ride?username=${encodeURIComponent(targetUsername)}`)
            if (res.ok) {
              const data = await res.json()
              if (data.ride) {
                ...
                if (data.ride.episodes) {
                 ...
                 const histRes = await fetch(`http://localhost:5001/get_all_rides?username=${encodeURIComponent(targetUsername)}`)
                 if (histRes.ok) {
  ...
  const handleStartNewRide = async () => {
      ...
      try {
          const res = await fetch(`http://localhost:5001/start_ride`, {
  ...
  const handleEndRide = async () => {
      try {
          await fetch(`http://localhost:5001/end_ride`, {
              method: 'POST', headers: {'Content-Type': 'application/json'},
              body: JSON.stringify({ride_id: activeRideId})
          })
          fetch(`http://localhost:5001/stop_camera`, { method: 'POST' }).catch(()=>{})
      ...
      <main className="container main-content animate-fade-in">
        ...
        {(() => {
          ...
          const mapCenter = mapEvents.length > 0 
            ...
            : [27.7172, 85.3240] // Default Kathmandu fallback
